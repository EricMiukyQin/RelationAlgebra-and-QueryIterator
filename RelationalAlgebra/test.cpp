            ...
            project(
                join(
                    rename(
						...
						NameMap({ {"from_user_id", "user_id"}, {"to_user_id", "to_user_id"}, {"message_id", "message_id"} })),
                    user),
	...
	Table *q2 =
		project(
			join(
				join(
					rename(
						select(user, q2_predicate),
						NameMap({ {"user_id", "from_user_id"}, {"username", "username"}, {"birth_date", "birth_date"} })),
					routing),
				message),
    ...
    Table *q3 =
		project(
			select(
			    join(
				    join(
					    rename(user, NameMap({ {"user_id", "to_user_id"}, {"username", "username"}, {"birth_date", "birth_date"} })),
				        routing),
			        message),
			    q3_predicate),
			ColumnNames{"username"});
	...
	Table *q4 =
		project(
			join(
				join(
					rename(
						select(user, q4_to_predicate),
						NameMap({ {"user_id", "to_user_id"}, {"username", "to_username"}, {"birth_date", "to_user_birth_date"} })),
					join(
						rename(
							select(user, q4_from_predicate),
							NameMap({ {"user_id", "from_user_id"}, {"username", "from_username"}, {"birth_date", "from_user_birth_date"} })),
						routing)),
				message),
			ColumnNames{ "send_date" });
	...
	Table *q5 =
		project(
			select(
				join(
					join(
						rename(user, NameMap({ {"user_id", "from_user_id"}, {"username", "username"}, {"birth_date", "birth_date"} })),
						routing),
					message),
				q5_predicate),
			ColumnNames{"username"});
	...
	Table *q6 =
		project(
			join(
				join(
					rename(
						select(user, q6_predicate),
						NameMap({ {"user_id", "from_user_id"}, {"username", "from_username"}, {"birth_date", "from_user_birth_date"} })),
					routing),
				rename(user, NameMap({ {"user_id", "to_user_id"}, {"username", "to_username"}, {"birth_date", "to_user_birth_date"} }))),
			ColumnNames{ "to_username" });
...
static void test_q7()
{
	Table *q7 =
		project(
			join(
				select(message, q7_predicate),
				join(
					join(
						rename(user, NameMap({ {"user_id", "from_user_id"}, {"username", "from_username"}, {"birth_date", "from_user_birth_date"} })),
						routing),
					rename(user, NameMap({ {"user_id", "to_user_id"}, {"username", "to_username"}, {"birth_date", "to_user_birth_date"} })))),
