		...
		unique(
			sort(
				project(
					select(
						nested_loops_join(
							nested_loops_join(table_scan(user), { 0 }, table_scan(routing), { 0 }), { 4 },
							table_scan(message), { 0 }),
						q2_predicate),
				{ 5 }), { 0 })
		); // Use a table_scan on username.
    CHECK(match(c2, q2));
    delete q2;
    delete c2;
}
		...
		unique(
			sort(
				project(
				    select(
					    nested_loops_join(
						    nested_loops_join(index_scan(tc, &username), { 0 }, table_scan(routing), { 0 }), { 4 },
						    table_scan(message), { 0 }),
					    q2_predicate),
				{ 5 }), { 0 })
		);
...
static void test_q3()
{
	...
	Iterator *q3 =
		project(
			select(
				nested_loops_join(
					nested_loops_join(table_scan(user), { 0 }, table_scan(routing), { 1 }), { 4 },
					table_scan(message), { 0 }),
				q3_predicate), { 1 });
...
static void test_q4()
{
	...
	Iterator *q4 =
		project(
			nested_loops_join(table_scan(message), { 0 },
				nested_loops_join(index_scan(tc, &froglet), { 0 },
					nested_loops_join(index_scan(tc, &unguiferous), { 0 }, table_scan(routing), { 0 }), { 3 }), { 6 }), { 1 });
